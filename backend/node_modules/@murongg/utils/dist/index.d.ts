 ...
 * @returns
 */
declare function isMaybeNumber(val: any): boolean;
/**
 ...
 * @returns
 */
declare function isIpv6(val: string): boolean;
/**
 * Check if the value is a ipv4
 * @param val
 * @returns
 */
declare function isIpv4(val: string): boolean;
/**
 ...
 * @returns
 */
declare function isIpAddress(val: string): boolean;
declare function isIpAddress(val: string, type: 'ipv4' | 'ipv6'): boolean;
/**
 ...
 * @returns
 */
declare const toNumber: (val: any) => number;
 ...
 * @returns
 */
declare function polling(fn: Fn<Promise<boolean> | boolean>, interval?: number, maxAttempts?: number): Promise<boolean>;
declare function polling(fn: Fn<Promise<boolean> | boolean>, interval?: (attemptTime: number) => number, maxAttempts?: number): Promise<boolean>;
/**
 ...
 * @param retries
 */
declare function retry<T>(fn: () => Promise<T>, retries: number): Promise<T>;
declare function retry<T, E extends Error>(fn: () => Promise<T>, condition: (error: E) => boolean, retries: number): Promise<T>;
 ...
 * Reset object value
 */
type ValueReset<T extends object, K extends keyof T, V> = {
    [P in keyof T]: P extends K ? V : T[P];
};
/**
 * Maybe Constructor
 */
type MaybeConstructor<T> = T extends Constructor<T> ? Constructor<T> : T;

export { type ArgumentsType, type Arrayable, type Awaitable, type Constructor, type ControlledPromise, type DeepMerge, type ElementOf, type Fn, type KeyofToArray, type MaybeConstructor, type MergeInsertions, type Nullable, type NullableArray, type NullableObject, type NullableObjectWithKeys, type PartitionFilter, RegexpRules, type SingletonPromiseReturn, type UnionToIntersection, type ValueReset, assert, at, batchInvoke, capitalize, clamp, clampArrayRange, clearUndefined, createControlledPromise, createPromiseLock, createSingletonPromise, debounce, deepMerge, deepMergeWithArray, ensurePrefix, ensureSuffix, flattenArrayable, getTypeName, hasOwnProperty, invoke, isBoolean, isBrowser, isDate, isDeepEqual, isDef, isEmail, isFunction, isIpAddress, isIpv4, isIpv6, isKeyOf, isMaybeNumber, isNull, isNumber, isObject, isRegExp, isString, isStringNumber, isTruthy, isURL, isUndefined, isWindow, last, lerp, mergeArrayable, move, noNull, noop, notNullish, notUndefined, objectEntries, objectKeys, objectMap, objectPick, p, partition, polling, randomStr, range, remap, remove, retry, sample, shuffle, slash, sleep, sum, tap, template, throttle, timestamp, to, toArray, toNumber, toString, uniq, uniqueBy };
